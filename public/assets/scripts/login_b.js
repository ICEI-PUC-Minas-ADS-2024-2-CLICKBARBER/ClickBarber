...
btnEntrar.addEventListener('click',async (event)=>{
    ...
    //verifica se ja tem uma barbearia logada
    valid = await verificaLogin(localStorage.getItem('token_b') , 'b');
    ...
    else if(await fazLogin(cnpj,senha)){
        window.location.href = "home.html"
    }
})
...
async function procuraDado(dado){
    ...
    const response =  await fetch(url+ '/barbearias/cnpj' , {
        method: "POST",
        headers: {'Content-Type' : 'application/json'},
        body: JSON.stringify({cnpj: dado})
    })
...
async function fazLogin(cnpj,senha){

    //faz a requisição para o backend
    const response = await fetch(url+'/login/barbearia' ,{
        method: 'POST',
        headers: {'Content-Type' : 'application/json'},
        body: JSON.stringify({
            cnpj: cnpj,
    ...
    if(!response.ok){
        ...
        localStorage.setItem('token_b', data.token);
        return true;
    }
}
